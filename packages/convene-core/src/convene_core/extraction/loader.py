...
:class:`~convene_core.extraction.abc.Extractor` implementations from:

- Python packages declaring ``convene.extractors`` entry points
...
ENTRY_POINT_GROUP = "convene.extractors"
...
class ExtractorLoader:
    ...
    def load_from_entry_points(self) -> list[str]:
        ...
        ``convene.extractors`` group.  Each entry point should point to an
    ...
    def load_from_file(self, path: str | Path, *, name: str | None = None) -> list[str]:
        ...
        module_name = f"_convene_custom_extractor_{file_path.stem}_{id(file_path)}"
