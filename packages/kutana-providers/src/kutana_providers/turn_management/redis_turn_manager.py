...
Key schema:
    kutana:{meeting_id}:turns:queue          ZSET  score=priority_ts, member="pid:hrid"
    kutana:{meeting_id}:turns:speaker        STRING  participant_id
    kutana:{meeting_id}:turns:speaker_since  STRING  float timestamp
    kutana:{meeting_id}:turns:hand:{pid}     STRING  hand_raise_id (presence key)
    kutana:{meeting_id}:turns:meta:{hrid}    HASH    participant_id, priority, topic, raised_at

The ``kutana:{meeting_id}:turns:`` namespace prefix ensures that all turn
...
from kutana_core.models.turn import (
    HandRaisePriority,
    QueueEntry,
    QueueStatus,
    RaiseHandResult,
...
class RedisTurnManager(TurnManager):
    ...
    def _queue_key(meeting_id: UUID) -> str:
        return f"kutana:{meeting_id}:turns:queue"

    @staticmethod
    def _speaker_key(meeting_id: UUID) -> str:
        return f"kutana:{meeting_id}:turns:speaker"

    @staticmethod
    def _since_key(meeting_id: UUID) -> str:
        return f"kutana:{meeting_id}:turns:speaker_since"

    @staticmethod
    def _hand_key(meeting_id: UUID, participant_id: UUID) -> str:
        return f"kutana:{meeting_id}:turns:hand:{participant_id}"

    @staticmethod
    def _hand_prefix(meeting_id: UUID) -> str:
        """Prefix for all hand keys in a meeting (used in Lua)."""
        return f"kutana:{meeting_id}:turns:hand:"

    @staticmethod
    def _meta_key(meeting_id: UUID, hand_raise_id: UUID) -> str:
        return f"kutana:{meeting_id}:turns:meta:{hand_raise_id}"
    ...
    async def clear_meeting(self, meeting_id: UUID) -> None:
        ...
        """
        r = await self._get_redis()
        pattern = f"kutana:{meeting_id}:turns:*"
