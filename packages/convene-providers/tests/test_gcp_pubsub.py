...
def _make_publisher(
    topic_path: str = "projects/test-project/topics/convene-test_topic",
) -> AsyncMock:
...
def _make_subscriber(
    subscription_path: str = "projects/test-project/subscriptions/convene-test_topic--workers",
    ...
    async def _list_subs(*args: Any, **kwargs: Any) -> Any:
        yield MagicMock(topic="projects/test-project/topics/convene-test_topic")
...
def bus(publisher: AsyncMock, subscriber: AsyncMock) -> Any:
    ...
    b = PubSubMessageBus(project_id="test-project", topic_prefix="convene-")
...
class TestPubSubImportGuard:
    ...
    def test_raises_import_error_when_pubsub_missing(self) -> None:
        ...
        with patch(
            "convene_providers.messaging.gcp_pubsub._PUBSUB_AVAILABLE", False
        ):
            from convene_providers.messaging.gcp_pubsub import _require_pubsub
...
class TestPubSubNaming:
    ...
    def test_topic_name_replaces_wildcards(self) -> None:
        from convene_providers.messaging.gcp_pubsub import _topic_to_pubsub_name

        result = _topic_to_pubsub_name("meeting.*.events", "convene-")
        assert "*" not in result

    def test_subscription_name_contains_topic_and_group(self) -> None:
        from convene_providers.messaging.gcp_pubsub import _subscription_name_for_group

        result = _subscription_name_for_group("meeting.events", "workers", "convene-")
...
class TestPubSubAck:
    ...
    async def test_ack_calls_acknowledge_for_group_subscription(
        ...
        bus._sub_paths["test.topic::workers"] = (
            "projects/test-project/subscriptions/convene-test_topic--workers"
        )
