...
def _make_sns_client(
    create_topic_arn: str = "arn:aws:sns:us-east-1:123456789012:convene-test-topic",
...
def bus(mock_session: tuple[MagicMock, MagicMock, MagicMock]) -> Any:
    ...
    with patch.dict("sys.modules", {"aioboto3": MagicMock()}):
        ...
        b = SQSMessageBus(region="us-east-1", topic_prefix="convene-")
        b._session = session
        return b
...
class TestSQSMessageBusImportGuard:
    ...
    def test_raises_import_error_when_aioboto3_missing(self) -> None:
        ...
        with patch.dict("sys.modules", {"aioboto3": None}):  # type: ignore[dict-item]
            ...
            if "convene_providers.messaging.aws_sns_sqs" in sys.modules:
                del sys.modules["convene_providers.messaging.aws_sns_sqs"]

            # Re-import with missing dependency simulated
            with patch(
                "convene_providers.messaging.aws_sns_sqs._AIOBOTO3_AVAILABLE", False
            ):
                from convene_providers.messaging.aws_sns_sqs import _require_aioboto3
...
class TestTopicNaming:
    ...
    def test_topic_to_sns_name_replaces_dots(self) -> None:
        from convene_providers.messaging.aws_sns_sqs import _topic_to_sns_name

        assert _topic_to_sns_name("meeting.abc.events", "convene-") == (
            "convene-meeting_abc_events"
        )

    def test_topic_to_sns_name_replaces_wildcards(self) -> None:
        from convene_providers.messaging.aws_sns_sqs import _topic_to_sns_name

        result = _topic_to_sns_name("meeting.*.events", "convene-")
        assert "*" not in result

    def test_queue_name_for_group_contains_topic_and_group(self) -> None:
        from convene_providers.messaging.aws_sns_sqs import _queue_name_for_group

        result = _queue_name_for_group("meeting.events", "workers", "convene-")
...
class TestSQSPublish:
    ...
    async def test_publish_creates_sns_topic(
        ...
        """publish() calls SNS create_topic for the given Convene topic."""
...
class TestSQSAck:
    ...
    async def test_ack_deletes_message_for_group_subscription(
        ...
        queue_name = "convene-test_topic--workers"
...
class TestCreateFromEnvAWS:
    ...
    def test_create_sqs_bus_from_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from convene_providers.messaging.aws_sns_sqs import SQSMessageBus
        from convene_providers.messaging.redis_streams import create_message_bus_from_env
