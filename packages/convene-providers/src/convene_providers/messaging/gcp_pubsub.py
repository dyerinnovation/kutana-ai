...
Uses the ``google-cloud-pubsub`` async client.  Each Convene topic maps
...
Requires ``google-cloud-pubsub`` (install with ``uv add 'convene-providers[gcp]'``).
...
Configuration environment variables::
    ...
    PUBSUB_TOPIC_PREFIX             Prefix for Pub/Sub topic names (default: convene-)
"""
...
def _require_pubsub() -> None:
    ...
    if not _PUBSUB_AVAILABLE:
        msg = (
            ...
            "Install it with: uv add 'convene-providers[gcp]'"
        )
        raise ImportError(msg)


def _topic_to_pubsub_name(topic: str, prefix: str) -> str:
    """Convert a Convene topic string to a valid Pub/Sub topic ID.
    ...
    Args:
        topic: Convene topic string (e.g. ``"meeting.abc.events"``).
...
class PubSubMessageBus(MessageBus):
    ...
    Args:
        ...
        topic_prefix: Prefix for Pub/Sub topic names. Defaults to ``"convene-"``.
    ...
    def _topic_path(self, topic: str) -> str:
        """Return the full Pub/Sub topic resource path for a Convene topic.
    ...
    async def publish(
        ...
        Args:
            topic: Convene topic string — maps to a Pub/Sub topic name.
    ...
    async def _consume_pattern(self, sub: Subscription) -> None:
        ...
        async for sub_resource in response:
            ...
            if fnmatch.fnmatch(topic_id, pattern_id):
                convene_topic = topic_id.removeprefix(self._topic_prefix)
                effective_group = sub.group or sub.subscription_id[:16]
                cache_key = f"{convene_topic}::{effective_group}"
                if cache_key not in self._sub_paths:
                    await self._get_or_create_subscription(
                        convene_topic, effective_group
                    )
                sub_path = self._sub_paths.get(cache_key)
                if sub_path:
                    await self._pull_and_dispatch(sub, sub_path)
