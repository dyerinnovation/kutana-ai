...
delivery.  Each Convene topic maps directly to a JetStream subject.  Consumer
...
Requires ``nats-py`` (install with ``uv add 'convene-providers[nats]'``).
...
def _require_nats() -> None:
    ...
    if not _NATS_AVAILABLE:
        msg = (
            ...
            "Install it with: uv add 'convene-providers[nats]'"
        )
        raise ImportError(msg)


def _topic_to_subject(topic: str) -> str:
    """Convert a Convene topic string to a NATS subject.

    Convene topics use dot-separated tokens (``"meeting.abc.events"``),
...
class NATSMessageBus(MessageBus):
    ...
    async def publish(
        ...
        Args:
            topic: Convene topic string — maps directly to a NATS subject.
