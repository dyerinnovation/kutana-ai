...
## Problem
Commits pushed to `dyerinnovation/kutana-ai` show as authored by `jcdpath` on GitHub because:
...
## Steps
1. Set repo-local git user config (`user.email` and `user.name`) for kutana-ai
