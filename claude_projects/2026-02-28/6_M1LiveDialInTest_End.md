...
- Created `scripts/dial_test.py` — standalone async script that:
  ...
  - Reads new events from Redis Stream `kutana:events`
...
- Redis Stream key is `kutana:events` with `event_type` and `payload` (JSON) fields — the script uses `XRANGE` to read all entries and filters by pre-test count
