# Platform Strategy: Why Kutana Must Own the Meeting Layer
...
Kutana isn't trying to out-feature Zoom. It's building the room where agents are the point.

This is the difference between competing with a platform and creating a new one. Slack didn't compete with email by being a better email client. It created a different kind of communication that happened to coexist with email, then ate email's lunch in team settings. Kutana does the same thing for meetings: build something new where agents are native, let critical mass develop, and let the traditional platforms eventually chase Kutana rather than the other way around.
...
2. **Integration puts Kutana in a permanent second-class position.** Bot users aren't real participants. They can be blocked, rate-limited, or de-listed. There's no path from "tolerated bot" to "first-class platform participant" without the platform's cooperation — which they will never grant.

3. **Integration doesn't differentiate.** If Kutana's value proposition is "join Zoom meetings," then Kutana is competing with Otter.ai and Fireflies.ai on commodity features (notes, summaries, task extraction). That's a race to the bottom on pricing with no durable moat.
...
Kutana's version of this playbook:
...
2. Get the first 1,000 developers. Each developer building an agent on Kutana is a force multiplier: their agent needs Kutana, which means anyone who wants to meet with that agent needs Kutana.
3. Build critical mass in the developer community until Kutana is the de facto standard for agent-native meetings.
4. Let traditional platforms come to Kutana with integration requests, not the other way around.
...
Kutana is not a mass-market product in Phase 1. Mass-market adoption requires the kind of inertia and switching cost that takes years to build. The go-to-market wedge is the developer community — specifically, developers building personal agents.

**Why developers:**
- They are underserved. No platform makes it easy to build a meeting-capable agent.
- They are force multipliers. One developer's agent can be the reason 10 other people use Kutana.
...
- Each agent-meeting involves at least one other human participant who now knows Kutana exists
- 20% organic referral rate → 1,000 new users/week at scale
- No paid acquisition needed in Phase 1

**The business philosophy:**
Many niche committed bases are more durable than one large casual base. A developer who has built their agent on Kutana is deeply committed. They don't churn when a competitor appears. They advocate for Kutana internally. They build integrations. Contrast with a casual notetaker user who switches to whatever has the best AI summary this week.
...
The cost of a developer using Kutana for free to build an agent is low (they're not running meetings 24/7). The value of that developer becoming an advocate, writing blog posts, and building in public is high. Don't charge for the thing that creates advocates.

---

## Recommendation

Own the platform. Become the standard for agent-native meetings. Let traditional platforms come to Kutana.

The market opportunity is real ($3.5B → $21.5B), the timing is right (personal agents are just emerging), the incumbent platforms have structural reasons not to build this, and the developer community is primed for a tool like Kutana.
