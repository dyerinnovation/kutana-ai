...
Convene is the meeting platform built for AI agents — where humans and AI collaborate in real-time, agents connect via a native API, and every commitment is tracked across meetings with persistent memory.
...
Convene is an agent-first meeting platform that serves two audiences:

### For Teams: Meetings That Actually Drive Accountability

Convene is where teams meet when they want AI that goes beyond transcription:
...
Convene is infrastructure that gives any AI agent the ability to participate in meetings:
...
- **MCP Server** — any MCP-compatible AI assistant (Claude, custom agents) can join Convene meetings through standard tool calls like `join_meeting`, `get_transcript`, `send_audio`
- **Python SDK** — `pip install convene` and connect an agent in 10 lines of code
...
- **OpenClaw** (openclaw.ai) — An open-source personal AI assistant could join your morning standup via Convene's MCP server and deliver your task updates on your behalf when you're unavailable.
...
- **Sales coaching agents** — A sales team's AI coach joins client calls on Convene, provides real-time suggestions in the sidebar (visible only to the rep), and generates follow-up action items.
...
OpenAI's Operator, Anthropic's computer use, thousands of custom agents. The AI agent market is projected to reach $43–53B by 2030 (38–46% CAGR). Every one of these agents eventually needs to participate in human meetings. No platform serves them today. The developers building these agents are hitting the exact problem Convene solves.
...
The Model Context Protocol (MCP) is emerging as the standard way AI assistants interact with tools and services. By exposing Convene as an MCP server, every MCP-compatible agent gets meeting access without custom integration work. This dramatically lowers the barrier for the agent ecosystem.
...
Developers at startups and enterprises building AI-powered tools that need meeting access. They currently resort to Twilio dial-in, headless browser bots, or Recall.ai. Convene's Agent API and MCP server give them a purpose-built solution. Think of this as the "Twilio for meetings" play — infrastructure that other products build on.

**Secondary: Mid-market teams ($10M–$500M revenue)**
Teams with 20–200 employees where meeting accountability is a real workflow gap. They adopt Convene for the built-in AI features (task extraction, memory, progress reporting) and stay for the platform.
...
Convene sits at the intersection of all three. The agent infrastructure market (developer API) has no direct equivalent today — Recall.ai ($250M valuation) is the closest proxy, but they provide access to other platforms' meetings rather than owning the meeting environment.
...
**Important pricing principle**: No minute caps. Users hate minute caps — it's the #1 complaint about Otter and Fireflies. Convene is unlimited meetings at every seat-based tier, with differentiation on features and team size.
...
1. **Platform ownership eliminates integration risk**: Unlike bolted-on tools, Convene can't be blocked by Zoom updating their API or Teams cracking down on bots. You own the meeting environment.
...
4. **Agent ecosystem moat**: As developers build agents for Convene's API, the platform accumulates a marketplace of specialized agents. Each agent makes the platform more valuable. Competitors would need to attract both developers AND users simultaneously.

5. **MCP-first positioning**: By being the first meeting platform to support MCP, Convene becomes the default meeting integration for the entire MCP ecosystem.

---

## The Founding Thesis

The meeting is where human work and AI work converge. Every commitment, decision, and status update passes through a meeting. By building the platform purpose-designed for that convergence — where AI agents connect as naturally as humans do — Convene becomes the operating layer for AI-augmented teams.

The compound effects are what matter: every meeting Convene hosts makes its agents smarter about your team, your commitments, and your patterns. The platform that never forgets a promise — and has the agents to enforce it — closes the gap between conversation and execution.

Convene doesn't compete with Zoom on video quality or with Otter on transcription accuracy. It competes on a different axis entirely: it's the only meeting platform where AI agents are first-class participants, and the only AI meeting tool that owns the environment its agents operate in.
