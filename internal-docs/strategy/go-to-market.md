...
Convene is pivoting from a meeting bot to an **agent-first meeting platform**. This unlocks two growth loops that reinforce each other:

**Growth Loop 1 (Developer-Centric):**
Developer builds an AI agent → needs reliable meeting access for the agent → discovers Convene API/MCP server → integrates Convene into their agent's workflow → agent joins Convene meetings natively → their agent gains access to a growing network of meetings → visibility increases → more teams adopt Convene to use these agents.

**Growth Loop 2 (Team-Centric):**
Team tries Convene for built-in AI features (task tracking, persistent memory, meeting assistant) → discovers the value of smart meeting intelligence → loves the outcome (less wasted meetings, more accountability) → invites more team members → entire org moves meetings to Convene → agents from Loop 1 are now available to this team at no additional friction.

The insight remains: **the product markets itself inside every meeting it joins.** But by owning the platform, we control the entire experience. Every meeting on Convene is a controlled, curated moment where AI value is showcased perfectly. An agent speaking a progress report is more memorable and convincing than any marketing dollar could buy.

The trust component remains critical. Meeting AI faces a serious legitimacy problem (Otter's lawsuit, user distrust). Convene grows through the same viral mechanism while positioning as the ethical, consent-first, developer-friendly alternative.
...
**Convene Solves**: Agents become first-class meeting participants via Convene's API and MCP server. A developer integrates Convene once and their agent gains:
...
1. **Developer documentation and quickstart**:
   ...
   - Five-minute quickstart: "Connect your agent to Convene in 5 lines of code"
   - Hosted docs with interactive examples

2. **Python SDK on PyPI**:
   - `pip install convene-ai` — the frictionless entry point
...
4. **MCP Server documentation**:
   - List Convene in official MCP server directories
   - Integration with Anthropic's tool registry
   - Docs on how to register a custom agent with Convene
...
**Expected Outcome**: 50–100 developers integrate Convene. Each integration drives teams to adopt Convene to access those agents. Developer virality amplifies team adoption.
...
**Convene Solves**: Convene is a meeting platform (not a bot overlay). Teams host meetings on Convene, and every meeting automatically extracts tasks, maintains persistent memory, and integrates into the team's workflow. Plus: agents from Loop 1 are available as first-class participants.
...
2. **Agent framework ecosystems** — LangChain, CrewAI, AutoGen. Get Convene listed in their integration docs. Developer-to-developer word-of-mouth.

3. **MCP ecosystem** — Listed in MCP server registries, Anthropic's tool registry, and other AI framework integrations.

4. **PyPI / npm** — SDK distribution channel. "I searched for 'meeting' on PyPI and found Convene."
...
10. **Zoom App Marketplace** — Even though Convene uses phone dial-in, a marketplace listing provides discovery and legitimacy.
...
- Found Convene because a direct report started using it OR an agent they want to access requires Convene
...
**Case studies from Phase 1 users**: Quantified results — "Team X reduced meeting time by 20% and increased task completion rate by 35% after 8 weeks with Convene."

**Self-serve annual plans**: Offer 20% discount for annual billing.

**Agent marketplace**: As agents from Loop 1 prove valuable, showcase them on Convene's marketplace. "Install these 3 agents to your Convene workspace."
...
**Demo video as primary asset**: Record a real standup where Convene's agent reports on last meeting's tasks. Video goes on landing page, Twitter, LinkedIn, Product Hunt update, HN.

**"Convene just spoke in my meeting" viral moment**: Genuine surprise creates organic social posts, Slack messages, word-of-mouth.
...
- LangChain integration docs (Convene as a "tool" for agents)
...
### PyPI / npm
- `pip install convene-ai` is the entry point for developers
...
- Partnership: "Use Recall for recording, Convene for agent coordination"
- Or: "Migrate your Recall workflows to Convene's agent-first platform"
- Existing customer relationships accelerate adoption

### Developer Relations
- Sponsor AI/ML conferences (not expenses, partnerships)
- Host "Agent Summit" — virtual meetup for Convene agent builders
- Feature developer stories on blog and social
- Quarterly "State of Convene Agents" report
...
- Number of unique agents running on Convene
...
**Convene AI** is the agent-first meeting platform. While other tools make transcripts, Convene makes agents first-class meeting participants. Build an agent once, access every meeting via Convene's API or MCP server — no platform lock-in, no SDK hacks. Your agents gain real-time visibility, can speak and influence decisions, and maintain persistent memory across conversations. **Unlike closed-garden solutions** (Zoom, Teams bots) that lock you into one platform, **Convene agents work everywhere** because they dial in via phone and coordinate via APIs.

### For Engineering Teams / Startup Founders

**Convene AI** is the meeting platform where AI actually works. Teams start using Convene because they need smarter meetings (automatic task tracking, persistent memory, action item accountability). They stay because the platform becomes the source of truth for what everyone committed to. As your team grows, the Convene agent marketplace provides off-the-shelf assistants to unlock even more value. **Unlike transcription tools** (Otter, Fireflies) that produce documents nobody reads, **Convene is a real platform** where tasks are tracked, memory persists, and agents help teams ship faster.
