"""Eval tests for Basic tier agents: Meeting Notetaker, Meeting Summarizer."""
...
from evals.mock_runner import run_mock_eval
...
BASIC_AGENTS = ["meeting-notetaker", "meeting-summarizer"]

_basic_scenarios: list[tuple[str, Scenario]] = []
for _agent_slug in BASIC_AGENTS:
    ...
    if _scenario_dir.exists():
        for _path in sorted(_scenario_dir.glob("*.json")):
            ...
            _basic_scenarios.append((_path.stem, _scenario))
...
@pytest.mark.parametrize(
    ...
    _basic_scenarios,
    ids=[f"{s.agent_template}/{name}" for name, s in _basic_scenarios],
)
async def test_basic_agent_mock(
    scenario_name: str,
    scenario: Scenario,
    ...
    Sends the agent's system prompt + transcript through the Messages API,
    captures tool_use blocks, and scores via LLM-as-Judge.
    """
    # Load system prompt
    ...
    # Judge
    from evals.mock_runner import format_transcript
...
@pytest.mark.parametrize(
    "scenario_name,scenario",
    [(name, s) for name, s in _basic_scenarios if "happy-path" in name],
    ids=[f"{s.agent_template}/{name}" for name, s in _basic_scenarios if "happy-path" in name],
)
async def test_basic_agent_e2e(
    scenario_name: str,
    scenario: Scenario,
    e2e_runner: object,
    all_rubrics: dict[str, Rubric],
    ...
    """Run E2E eval for a Basic tier agent happy-path scenario.

    Creates a real meeting on the dev cluster, activates the agent,
    injects transcript, observes events, and scores.
    """
    ...
    # Create meeting and activate agent
    ...
    try:
        ...
        # Format events as agent response for judging
